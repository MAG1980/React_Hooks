...
Если состояние компонента изменилось, то по-умолчанию считается, что все его дочерние компоненты также нуждаются в перерисовке.
...
Чтобы отследить что и когда рендерится в приложении можно использовать расширения для браузера или использовать логирование рендеров.


Данные в React-приложении обычно передаются сверху вниз с помощью props.
Исключенимем являются глобальные данные, которые могут требоваться в нескольких компонентах на самых разных уровнях дерева.
...
В связке с useContext часто упоминают хук useReducer.
